...
class Visualizer():
    def __init__(self):
        ...
        self.draw_trajectory_guide = False  # 2026 默认不绘制车头轨迹引导线
    
    def replay_result(self, result_path: str, save_path: str=None, scene_info: ScenarioInfo=None, draw_trajectory_guide: bool=False):
        """可视化回放接口"""
        self.draw_trajectory_guide = bool(draw_trajectory_guide)
        # 解析结果文件
        ...
        if scene_info is not None:
            ...
            task = self._parse_task_from_result_file(result_file)
    ...
    @staticmethod
    def _parse_task_from_result_file(result_file: str) -> str:
        """从结果文件名中解析场景名，兼容 REPLAY_*_av*_scenario_xxx_result.csv"""
        match = re.search(r"(scenario_[0-9a-zA-Z]+)_result\.csv$", result_file)
        if match:
            return match.group(1)

        # 回退到老规则（例如 REPLAY_0_xxx_result.csv）
        parts = result_file.split('_')
        if len(parts) >= 4:
            return '_'.join(parts[2:-1])
        raise ValueError(f"Failed to parse task from result file: {result_file}")
