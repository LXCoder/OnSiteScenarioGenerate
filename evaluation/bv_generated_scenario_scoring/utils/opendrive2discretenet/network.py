...
from .utils import encode_road_section_lane_width_type_id
        ...
        for road in opendrive.roads:
            for lane_section in road.lanes.lane_sections:
                for lane in lane_section.allLanes:
                    ...
                    # )
                    # 对参数化车道的ID信息进行编码，输出'<road.id>.<lane_section.id>.<lane.id>.<width.id>.<type.id>'
                    parametric_lane_id = encode_road_section_lane_width_type_id(
                        road.id, lane_section.idx, lane.id, -1, 0
                    )
                    '''对该Road所包含各section（除去最后一个section）对应后继successor信息进行编码
                    输出'<road.id>.<section_successor.id>.<lane_successor.id>.<width.id>.<type.id>'
                    ...
                    if lane_section.idx < road.lanes.getLastLaneSectionIdx():
                        ...
                        # )
                        successorId = encode_road_section_lane_width_type_id(
                            road.id, lane_section.idx + 1, lane.link.successorId, -1, 0
                        )
                    ...
                    # Try to get next road
                        ...
                        if next_road is not None:
                            ...
                            if road.link.successor.contactPoint == "start":
                                ...
                                # )
                                successorId = encode_road_section_lane_width_type_id(
                                    next_road.id, 0, lane.link.successorId, -1, 0
                                )
                    ...
                    输出'<road.id>.<section_predecessor.id>.<lane_predecessor.id>.<width.id>.<type.id>'
                    ...
                    if lane_section.idx > 0:
                        ...
                        # )
                        predecessorId = encode_road_section_lane_width_type_id(
                            road.id, lane_section.idx - 1, lane.link.predecessorId, -1, 0
                        )
                    ...
                    # Try to get previous road
                        ...
                        if prevRoad is not None:
                            ...
                            if road.link.predecessor.contactPoint == "start":
                                ...
                                # )
                                predecessorId = encode_road_section_lane_width_type_id(
                                    prevRoad.id, 0, lane.link.predecessorId, -1, 0
                                )
                            else:  # contact point = end
                                # predecessorId = encode_road_section_lane_width_id(
                                #     prevRoad.id,
            ...
            for connection in junction.connections:
                ...
                for lane_link in connection.laneLinks:
                    if contact_point == "start":
                        ...
                        # 对incoming_road进行id重编码，编码结构：<road.id>.<lane_section.id>.<lane.id>.<width.id>.<type.id>
                        ...
                        # )
                        incoming_road_id = encode_road_section_lane_width_type_id(
                            incoming_road.id, lane_section_idx, lane_link.fromId, -1, 0
                        )
                        # connecting_road_id = encode_road_section_lane_width_id(
                        #     connecting_road.id, 0, lane_link.toId, -1
                        # )
                        connecting_road_id = encode_road_section_lane_width_type_id(
                            connecting_road.id, 0, lane_link.toId, -1, 0
                        )
                        ...
                        # )
                        incoming_road_id = encode_road_section_lane_width_type_id(
                            incoming_road.id, 0, lane_link.fromId, -1, 0
                        )
                        # connecting_road_id = encode_road_section_lane_width_id(
                        #     connecting_road.id,
