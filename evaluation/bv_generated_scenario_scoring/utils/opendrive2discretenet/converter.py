...
class OpenDriveConverter:
    ...
    def lane_section_to_parametric_lanes(
        ...
        for side in ["right", "left"]:
            ...
            for lane in lanes:
                ...
                # )
                plane_group = ParametricLaneGroup(
                    id_=encode_road_section_lane_width_type_id(
                        lane_section.parentRoad.id, lane_section.idx, lane.id, -1, 0
                    ),
                    inner_neighbour=inner_neighbour_id,
                    inner_neighbour_same_direction=inner_neighbour_same_dir,
                    outer_neighbour=outer_neighbour_id,
    ...
    def determine_neighbours(lane) -> Tuple[str, str, bool]:
        ...
        # )
        inner_neighbour_id = encode_road_section_lane_width_type_id(
            lane.lane_section.parentRoad.id, lane.lane_section.idx, inner_lane_id, -1, 0
        )
        ...
        # )
        outer_neighbour_id = encode_road_section_lane_width_type_id(
            lane.lane_section.parentRoad.id, lane.lane_section.idx, outer_lane_id, -1, 0
        )
