...
class Player(FuncAnimation):
    ...
    def setup(self, playerax):
        ...
        self.button_back.on_clicked(self.onebackward)
...
class Visualizer():
    def __init__(self):
        ...
        self.dt = 0.1               # 时间步长，用于轨迹预测
        # SVG保存相关属性
        self.save_svg = False       # 是否保存SVG
        self.svg_save_dir = None    # SVG保存目录
        self.svg_filename_prefix = "frame"  # SVG文件名前缀
    
    def replay_result(self, result_path: str, save_path: str=None, save_svg: bool=False, svg_save_dir: str=None, svg_filename_prefix: str="frame"):
        """可视化回放接口
        Args:
            result_path: 结果文件路径
            save_path: 动画保存路径（可选）
            save_svg: 是否保存每一帧的SVG图像
            svg_save_dir: SVG保存目录
            svg_filename_prefix: SVG文件名前缀
        """
        # 设置SVG保存选项
        self.save_svg = save_svg
        self.svg_save_dir = svg_save_dir
        self.svg_filename_prefix = svg_filename_prefix
        
        # 如果启用SVG保存，创建保存目录
        if self.save_svg and self.svg_save_dir:
            os.makedirs(self.svg_save_dir, exist_ok=True)
    ...
    def replay_update(self, frame: int):
        ...
        self.update_dynamic(test_info, observation.ego_info, observation.object_info, frame)
        
        # 如果启用SVG保存，保存当前帧的SVG图像
        if self.save_svg and self.svg_save_dir:
            self._save_frame_as_svg(frame)

    def update_dynamic(self, test_info: dict, ego_info: EgoStatus, object_info: Dict[str, ObjectStatus], current_frame: int = None):
        """对每一帧进行画布更新"""
        ...
        if current_frame is not None:
            self._plot_single_object_with_trajectory_from_data(self.ax_detail_obj, 'ego', ego_info, current_frame, c=self.object_color['ego'])
            for obj_type in object_info.keys():
                for obj_id, obj_state in object_info[obj_type].items():
                    self._plot_single_object_with_trajectory_from_data(self.ax_detail_obj, obj_id, obj_state, current_frame, c=self.object_color[obj_type])
        else:
            # 如果没有帧索引，使用预测方法作为备选
            self._plot_single_object_with_trajectory(self.ax_detail_obj, 'ego', ego_info, c=self.object_color['ego'])
            for obj_type in object_info.keys():
                for obj_id, obj_state in object_info[obj_type].items():
                    self._plot_single_object_with_trajectory(self.ax_detail_obj, obj_id, obj_state, c=self.object_color[obj_type])
        
        # 更新左下角地图区域定位框位置
        if new_range:
            self.position_box.set_xy([new_range[0][0], new_range[1][0]])

    def _predict_trajectory(self, x0: float, y0: float, v0: float, yaw0: float, dt: float = 0.1, steps: int = 5):
        """预测车辆未来5步的轨迹
        Args:
            x0, y0: 当前位置
            v0: 当前速度
            ...
            steps: 预测步数
        Returns:
            trajectory: 预测轨迹点坐标列表 [(x1, y1), (x2, y2), ...]
        """
        trajectory = []
        x, y, v, yaw = x0, y0, v0, yaw0
    ...
    def _plot_single_object_with_trajectory(self, ax, key: str, objecti: ObjectStatus, c='cornflowerblue'):
        """绘制车辆及其未来轨迹"""
        # 绘制车辆本体
        self._plot_single_object(ax, key, objecti, c)
        
        # 从数据中读取未来轨迹（需要知道当前帧索引）
        # 这里我们需要在调用时传入当前帧索引
        # 暂时使用预测方法作为备选
        trajectory = self._predict_trajectory(
            objecti.x, objecti.y, objecti.v, objecti.yaw, 
            dt=self.dt, steps=10
        )
        ...
        if trajectory:
            ...
            # 绘制轨迹线，使用透明色
            ax.plot(traj_x, traj_y, color=c, alpha=0.8, linewidth=2, linestyle='-', zorder=3)
            
            # # 绘制轨迹点，使用透明色
            # ax.scatter(traj_x, traj_y, color=c, alpha=0.3, s=20, zorder=3)

    def _plot_single_object_with_trajectory_from_data(self, ax, key: str, objecti: ObjectStatus, current_frame: int, c='cornflowerblue'):
        """从数据中读取并绘制车辆及其未来轨迹"""
        # 绘制车辆本体
        self._plot_single_object(ax, key, objecti, c)
        
        # 从数据中读取未来轨迹
        trajectory = self._get_future_trajectory_from_data(current_frame, key, steps=10)
    ...
    def _get_frame_info_from_result(self, frame: int) -> dict:
        ...
        test_info = {
            ...
            'acc': f"{new_observation.ego_info.a} ({round(frame_series.get('acc', ''), 3)})" if 'acc' in frame_series else f"{new_observation.ego_info.a}",
            'rot': f"{new_observation.ego_info.rot} ({round(frame_series.get('rot', ''), 3)})" if 'rot' in frame_series else f"{new_observation.ego_info.rot}",
    ...
    def _save_frame_as_svg(self, frame: int):
        """保存当前帧为SVG图像
        Args:
            frame: 当前帧索引
        """
        if not self.save_svg or not self.svg_save_dir:
            return
            
        try:
            # 生成SVG文件名
            svg_filename = f"{self.svg_filename_prefix}_{frame:04d}.svg"
            svg_path = os.path.join(self.svg_save_dir, svg_filename)
            
            # 保存当前画布为SVG
            self.fig.savefig(svg_path, format='svg', bbox_inches='tight', dpi=300)
            
            # 可选：打印保存信息
            print(f"已保存第{frame}帧SVG图像: {svg_path}")
            
        except Exception as e:
            print(f"保存第{frame}帧SVG图像失败: {e}")

    def _get_future_trajectory_from_data(self, current_frame: int, obj_id: str, steps: int = 10):
        """从数据中直接读取车辆未来轨迹
        Args:
            current_frame: 当前帧索引
            obj_id: 车辆ID
            steps: 未来步数
        Returns:
            trajectory: 未来轨迹点坐标列表 [(x1, y1), (x2, y2), ...]
        """
        trajectory = []
        
        # 检查是否有足够的未来帧数据
        if current_frame + steps >= len(self.result_df):
            steps = len(self.result_df) - current_frame - 1
        ...
        for i in range(1, steps + 1):
            ...
            if future_frame < len(self.result_df):
                # 尝试读取未来帧的数据
                try:
                    if obj_id == 'ego':
                        x = self.result_df.iloc[future_frame]['x_ego']
                        y = self.result_df.iloc[future_frame]['y_ego']
                    else:
                        x = self.result_df.iloc[future_frame][f'x_{obj_id}']
                        y = self.result_df.iloc[future_frame][f'y_{obj_id}']
