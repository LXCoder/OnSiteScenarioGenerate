...
class DataRecord:
    ...
    def merge_frame(self) -> pd.DataFrame:
        ...
        """
        # Keep merge idempotent for realtime flush: avoid repeatedly appending "_ego".
        ego_frame = self.ego_data.copy()
        ego_frame.columns = [c if str(c).endswith("_ego") else f"{c}_ego" for c in ego_frame.columns]
        ...
        return reduce(
            ...
            [self.control_data, ego_frame, *object_dataframe_group, self.end_data],
        )
