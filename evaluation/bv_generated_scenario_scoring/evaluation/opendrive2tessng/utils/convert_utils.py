...
def convert_opendrive(opendrive: OpenDrive, filter_types: List, roads_info: Dict, context: Dict = None) -> Scenario:
    ...
    benchmark_id = None
    return road_network.export_commonroad_scenario(benchmark_id=benchmark_id, filter_types=filter_types, roads_info=roads_info, context=context)
...
def convert_roads_info(opendrive: OpenDrive, step_length: int,
    ...
    for road in opendrive.roads:
        ...
        for section in road.lanes.lane_sections:
            ...
            # if road.id == 274:
            #     print("wy111", road_points[section_id])
            # print("wy111", road.id, road_points[section_id]["right_points"])#, road_points[section_id]["left_points"])
            # ea= []
            # for i in range(len(points)):
            #     angele = points[i]['angle']
            #     ea.append([angele])
            # print("wyqqq",ea)
        # # 计算每一段section 的高程信息
        # # 获取高程分段列表
        ...
        # if int(road.id) == 392:
        #     print("wy111", road.id, roads_info[road.id])
    return roads_info
...
def convert_lanes_info(opendrive: OpenDrive, scenario: Scenario, roads_info: Dict) -> Dict:
    ...
    for lane in scenario.lanelet_network.lanelets:
        ...
        # print("wy1111", lane.stop_line)
        # if int(lanes_info[lane.lanelet_id]['road_id']) == 388:
        # print("wy222", lane.lanelet_id, "wy", int(lanes_info[lane.lanelet_id]['road_id']), "wy",len(lanes_info[lane.lanelet_id]["road_marks"]))
        # if int(lanes_info[lane.lanelet_id]['road_id']) == 151:
        #     print("wy222", lane.lanelet_id, lanes_info[lane.lanelet_id])
        # if len(lanes_info[lane.lanelet_id]['road_marks']) != 0:
        #     for i in range(len(lanes_info[lane.lanelet_id]['road_marks'])):
        #         for key,value in (lanes_info[lane.lanelet_id]['road_marks'])[i].items():
        #             if value == "crosswalk":
        #                 print("wy222", lane.lanelet_id, lanes_info[lane.lanelet_id])
    # 车道ID，中心车道为0， 正t方向升序，负t方向降序(基本可理解为沿参考线从左向右下降)
    #     if lanes_info[lane.lanelet_id]["lane_id"] == 0:
    #         print("wy1111", lanes_info[lane.lanelet_id]["lane_id"])
    return lanes_info
